...
Napi::Value RegSetKeyValueWrap(const Napi::CallbackInfo &info) {
  try {
    if (info.Length() != 4) {
      throw Napi::Error::New(info.Env(), "Expected four parameters (key, subkey, name, data)");
    }
    #ifdef _WIN32
...
Napi::Value RegGetValueWrap(const Napi::CallbackInfo &info) {
  try {
    if (info.Length() != 3) {
      throw Napi::Error::New(info.Env(), "Expected three parameters (key, subkey, value)");
    }
    #ifdef _WIN32
    HKEY key = decodeKeyParameter(info.Env(), info[0]);
...
Napi::Value RegEnumKeysWrap(const Napi::CallbackInfo &info) {
  try {
    if (info.Length() != 1) {
      throw Napi::Error::New(info.Env(), "Expected one parameters (key)");
    }

    if (!info[0].IsBuffer()) {
      throw Napi::Error::New(info.Env(), "Parameter expected to be a buffer (as generated by WithRegOpen)");
    }
    #ifdef _WIN32
    HKEY key;
